...
var (
	...
	username   = "d.ganiuly@astanait.edu.kz"
	password   = "TonyStark27"
	portNumber = "587"
...
func main() {
	...
	fmt.Println(email)
	sendEmail := &Barcode{}
	fmt.Println(sendEmail.SendEmailWithPDF(email))
}
...
//	}
//
//	code := files[i].Filename
//	codeBar := code[:len(code)-4]
//	email := codeBar + "@astanait.edu.kz"
//
//
//
...
func (b *Barcode) SendEmailWithPDF(email string) string{
	pathToPdf := "scan1.PDF"
	e := NewEmail()
	e.From = username
	e.To = []string{email}
